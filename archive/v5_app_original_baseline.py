...
st.set_page_config(page_title="Photo Detective v5", layout="wide")
st.title("📸 Photo Detective: v5 Restoration")
st.markdown("Reverting to the last known working logic.")
...
with st.sidebar:
    ...
    sim_threshold = st.slider(
        ...
        max_value=30, 
        value=16,
        help="Higher = Looser matching. Lower = stricter. 16 is recommended."
    )
    
    search_radius = st.slider(
        "Time Search Radius (Days)",
        min_value=1,
        max_value=30,
        value=10,
        help="Only compare images taken within this many days of each other."
    )
...
# --- CORE LOGIC (EXACTLY AS UPLOADED) ---
...
def create_collage(cluster_data, cluster_id, output_folder):
    """Generates a side-by-side strip, saves it, and returns it for display."""
    images = []
    try:
        # Try to load a font, fallback if needed
        try:
    ...
    for item in sorted_cluster:
        try:
            ...
            x, y = 20, 20
            # Simple shadow/outline for text visibility
...
if run_button:
    if not os.path.exists(source_folder):
        ...
        # 2. Scan and Separate (Images vs Others)
        ...
        st.info(f"Found {len(all_image_files)} images to analyze.")
        if len(non_image_files) > 0:
            st.warning(f"Found {len(non_image_files)} non-image files (Videos/Docs). These will be copied safely to 'Keep'.")
        ...
        for i, f in enumerate(all_image_files):
            ...
            progress_bar.progress((i + 1) / len(all_image_files))
        ...
        # 6. PROCESS ORPHANS (The Single Images)
        singles_count = 0
        ...
        # 7. PROCESS NON-IMAGES (The Videos/Docs)
        non_image_count = 0
        ...
        with col1:
            st.metric("Moved to Trash", trash_count)
        with col2:
            st.metric("Cluster Winners", cluster_winners_count)
        with col3:
            st.metric("Orphans (Saved)", singles_count)
        with col4:
            st.metric("Videos/Other (Saved)", non_image_count)
            
        st.info(f"📂 **Clean Room:** Your 'Keep' folder now has **{total_kept}** files.")
        st.warning(f"🖼️ **Review:** Check `sorted_photos/Review_Collages` for saved comparisons.")