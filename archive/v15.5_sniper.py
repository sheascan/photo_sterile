...
def get_pattern_prefix(filename):
    ...
    # REFINED REGEX:
    # 1. ^(.{2,})  -> Capture at least 2 characters at the start
    # 2. [-_]      -> Followed by a separator
    # 3. \d+       -> Followed by digits
    match = re.match(r'^(.{2,})[-_]\d+', filename)
    
    if match:
        prefix = match.group(1)
        # Extra safety: If it's purely alpha and len < 2, ignore (redundant but safe)
...
def scan_database_for_targets():
    ...
    print("   (Ignoring single-letter prefixes)\n")
    ...
    print(f"{'ID':<4} | {'PREFIX':<20} | {'TOTAL FILES':<12} | {'DUPLICATES':<10}")
    print("-" * 55)
