...
st.set_page_config(page_title="Photo Detective v16.3", layout="wide")
st.title("🧐 Photo Detective v16.3: Speed Librarian")
    ...
    if st.session_state.scan_complete:
        ...
        if count > 0:
            ...
            if st.button(f"📦 Move {count} to Archive", type="primary"):
                ...
                # --- PRUNE HERE ONLY (When Moving) ---
                # We filter the list just before action to avoid errors
...
if not st.session_state.scan_complete:
    ...
    if not candidates:
        ...
        # --- OPTIMIZED GRID ---
        # We only render the first 20 images to keep the UI snappy
        # The list is huge, but humans only look at the top page anyway
        
        PAGE_SIZE = 20
        visible_candidates = candidates[:PAGE_SIZE]
        ...
        for idx, item in enumerate(visible_candidates):
            ...
            with col:
                try:
                    ...
                    # --- INSTANT ACTION ---
                    # Since we removed the global prune check, this button is fast again
                    if st.button("Ignore (Keep)", key=path):
                        # We remove by value, not index, to be safe
                        st.session_state.doc_candidates.remove(item)
                        st.rerun()
                except Exception:
                    # If file is gone, auto-remove it from state silently
                    if item in st.session_state.doc_candidates:
                        st.session_state.doc_candidates.remove(item)
                        st.rerun()

        if len(candidates) > PAGE_SIZE:
            st.info(f"... and {len(candidates) - PAGE_SIZE} more below. (Reviewing top 20 for speed)")