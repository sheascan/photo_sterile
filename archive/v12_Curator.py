...
# Updated to v12
st.set_page_config(page_title="Photo Detective v12", layout="wide")
st.title("🎨 Photo Detective v12: The Curator")
...
def get_db_stats():
    try:
        ...
        conn.close()
        return img_count, cluster_count
    except: return 0, 0
...
# Load Stats
total_imgs, total_clusters = get_db_stats()
    ...
    if total_clusters == 0:
        ...
        with c1: 
            if st.button("⬅️ Previous") and st.session_state.page > 0: 
                ...
                st.rerun()
        with c2: 
            st.markdown(f"**Viewing Page {st.session_state.page + 1} of {total_pages}** (Total Clusters: {total_clusters})")
        with c3:
...
with t2:
    st.header("Tools")
    st.write(f"Library Status: {total_imgs} Images | {total_clusters} Clusters")
